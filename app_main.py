...
def is_soybean_leaf(image_path):
    ...
    try:
        ...
        if width < 200 or height < 200:
            return False, "❌ Soyabean image not found, Please upload a proper soyabean leaf image."
        
        # Check aspect ratio (leaf-like proportions)
        aspect_ratio = width / height
        if aspect_ratio < 0.5 or aspect_ratio > 3.0:
            return False, "❌ Image doesn't have leaf-like proportions. Please upload a proper soybean leaf image."
        ...
        if contrast < 25:
            return False, "❌ Image quality is too poor. Please upload a clear, well-lit soybean leaf image."
        ...
        if total_color == 0:
            return False, "❌ Invalid image. Please upload a proper soybean leaf photo."
        ...
        if green_ratio < 0.35:  # Increased threshold for soybean leaves
            return False, "❌ This doesn't appear to be a soybean leaf. Soybean leaves are predominantly green."
        
        # Check if it's too red (fruits, flowers, etc.)
        if red_ratio > 0.45:
            return False, "❌ This appears to be a flower/fruit image. Please upload a soybean LEAF image."
        
        # Check if it's too blue (sky, water, etc.)
        if blue_ratio > 0.4:
            return False, "❌ This doesn't appear to be a leaf image. Please upload a soybean leaf."
        
        # Enhanced green content analysis
        # Convert to HSV-like color analysis
        max_channel = np.max(avg_color)
        if max_channel == 0:
            return False, "❌ Invalid image detected."
        ...
        if normalized_color[1] < 0.7:  # Green channel should be dominant
            return False, "❌ This image doesn't match soybean leaf characteristics."
        
        # Check for monochrome/grayscale images
        color_variance = np.std(avg_color)
        if color_variance < 15:
            return False, "❌ Image lacks color variation. Please upload a color photo of a soybean leaf."
        
        # File size and quality checks
        file_size = os.path.getsize(image_path)
        if file_size < 20 * 1024:  # Less than 20KB
            return False, "❌ Image file is too small (likely corrupted). Please upload a proper soybean leaf image."
        
        if file_size > 15 * 1024 * 1024:  # Larger than 15MB
            return False, "❌ Image file is too large. Please upload an image under 16MB."
        ...
        if edge_variance < 100:  # Very smooth images (walls, papers, etc.)
            return False, "❌ This doesn't appear to be a leaf. Leaves have visible texture and veins."
        ...
        if not is_likely_leaf:
            return False, "❌ This image cannot be processed as a soybean leaf. Please upload a clear photo of a soybean leaf."
        
        return True, "✅ Soybean leaf validation passed"
        
    except Exception as e:
        print(f"Validation error: {e}")
        return False, f"❌ Error processing image: {str(e)}. Please upload a valid soybean leaf image."
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            print(f"✅ User found in DB - ID: {user[0]}, Username: {user[1]}")
            print(f"📝 Stored hash: {user[3]}")
            print(f"🔑 Password being checked: {password}")
...
def upload_file():
    if 'file' not in request.files:
        flash('❌ No file selected. Please choose a soybean leaf image to upload.')
        return redirect(url_for('index'))
    ...
    if file and allowed_file(file.filename):
        ...
        flash('❌ Invalid file type. Please upload only JPG, PNG, or WEBP images of soybean leaves (Max 16MB).')
        return redirect(url_for('index'))
